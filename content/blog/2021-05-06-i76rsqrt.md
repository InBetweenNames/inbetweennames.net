...
=&\floor{-\frac{\fexp}{2}}\qquad\qquad\qquad\qquad(\mathrm{from}\ (1))\\
=&-\frac{\fexp}{2} - \frac{1}{2}\qquad\qquad\qquad(\mathrm{\fexp\ is\ odd}) \\
=&-\frac{\fexp + 1}{2}
{{< /katex >}}
...
But remember, we still need to bias the exponent!  That means we need to add the value $1023$ to it.
Let {{< katex >}}\biased{x} = x + 1023{{< /katex >}} denote a value biased by $1023$.  We have three cases:

{{< katex display >}}
\biased{r_\fexp} = \begin{cases}
  -\frac{\fexp + 1}{2} + 1023 &\text{if } \fexp \text{ is odd } \\
  -\frac{\fexp}{2} + 1023 &\text{if } \fexp \text{ is even and } \ffrac = 0 \\
  -\frac{\fexp + 2}{2} + 1023 &\text{if } \fexp \text{ is even and } \ffrac \neq 0 \\
\end{cases}
{{< /katex >}}
...
{{< katex display >}}
\biased{r_\fexp} = \begin{cases}
  -\frac{\biased{\fexp} - 1023 + 1}{2} + 1023 &\text{if } \fexp \text{ is odd } \\
  -\frac{\biased{\fexp} - 1023}{2} + 1023 &\text{if } \fexp \text{ is even and } \ffrac = 0 \\
  -\frac{\biased{\fexp} - 1023 + 2}{2} + 1023 &\text{if } \fexp \text{ is even and } \ffrac \neq 0 \\
\end{cases}
{{< /katex >}}
...
{{< katex display >}}
\biased{r_\fexp} = \begin{cases}
  \frac{3068 - \biased{\fexp}}{2} &\text{if } \fexp \text{ is odd } \\
  \frac{3069 - \biased{\fexp}}{2} &\text{if } \fexp \text{ is even and } \ffrac = 0 \\
  \frac{3067 - \biased{\fexp}}{2} &\text{if } \fexp \text{ is even and } \ffrac \neq 0 \\
\end{cases}
{{< /katex >}}
...
{{< katex display >}}
\biased{r_\fexp} = \begin{cases}
  \frac{\mathrm{0xBFC} - \biased{\fexp}}{2} &\text{if } \fexp \text{ is odd } \\
  \frac{\mathrm{0xBFD} - \biased{\fexp}}{2} + 1023 &\text{if } \fexp \text{ is even and } \ffrac = 0 \\
  \frac{\mathrm{0xBFB} - \biased{\fexp}}{2} + 1023 &\text{if } \fexp \text{ is even and } \ffrac \neq 0 \\
\end{cases}
{{< /katex >}}

Hey, wait a minute! That looks a *lot* like the `0xBFC` constant in the code, doesn't it?!
...
<div class="highlight">
<pre class="chroma"><code class="language-nasm" data-lang="nasm"><span class="mf">0</span>     <span class="k">eeeeeeeeeee</span>
^     ^
sign  exponent</code></pre></div>

Just the exponent bits.  This means `0xbfc - (param_1._4_4_ >> 0x14)` $ = \mathrm{0xBFC} - \biased{\fexp}$.

Next, the right shift.  This is equivalent to division by 2, meaning:

`(0xbfc - (param_1._4_4_ >> 0x14) >> 1)` $= \frac{\mathrm{0xBFC} - \biased{\fexp}}{2}$, our *odd* case.

Next, this is shifted back up with `<< 0x14`, giving us the following upper 32 bits of a `float64`:

<div class="highlight">
<pre class="chroma"><code class="language-nasm" data-lang="nasm"><span class="mf">0</span>     0<span class="k">eeeeeeeeee</span> <span class="kt">0000000000000000000000000</span>
^     ^           ^
sign  exponent    mantissa</code></pre></div>
...
There are a number of ways to do this.  To keep it simple, they chose to hard code one path here, always selecting the *odd* case.  It was probably
chosen probabilistically: the *odd* case covers half of the possible inputs, while the *even* cases split the remaining half depending on if $\ffrac = 0$.   And besides, this is just supposed to be an
initial guess anyway -- it doesn't need to be perfect.

We have now solved for the exponent bits of our initial guess and placed them in their correct positions.

<div class="highlight">
<pre class="chroma"><code class="language-nasm" data-lang="nasm"><span class="mf">0</span>     <span class="k">0eeeeeeeeee</span> <span class="kt">????????????????????????????????????????????????????</span>
^     ^           ^
sign  exponent    mantissa</code></pre></div>
