...
Note that this is a "one way" mapping from floating point values to reals.  Taking an arbitrary real number and converting it to a `float64` requires certain [conditions](#ieee754-real) to be met.
...
Let's recap.  Let $r_\fexp$ denote the exponent for the initial guess.  We have three cases:
...
Now we can directly determine what the exponent should be for the initial guess.  What is especially cool
...
Our *even* and $\ffrac \neq 0$ case!  Unfortunately $\floor{-\frac{0 + 1}{2}} = -1$, so this wouldn't work for the even and $\ffrac = 0$ case.
...
So `param_1._4_4_ >> 0x14` translates to "shift the upper 32 bits of the float64 right 20 bits", meaning "strip out the mantissa bits", leaving us with:
<div class="highlight">
...
Just the exponent bits.  Therefore:
...
initial guess anyway -- it doesn't need to be perfect.  Later, we'll see how this choice was compensated for in the generation of the lookup table.
...
In this case, the floating point exponent of the guess is $\floor{-\dfrac{\fexp + 1}{2}} = -\dfrac{\fexp + 1}{2}$.  Let's work backwards to get there:
...
{{< katex display >}}
 2^{\floor{-(\fexp + 1)/2}}(1+m)
{{< /katex >}}

is a valid IEEE-754 floating point value!  We have solved the mantissa for the odd case.
Next we will address the even cases.

### $\fexp$ is even and $\ffrac = 0$

Here, we know the floating point exponent of the guess is $-\dfrac{\fexp}{2}$.  Since $\ffrac = 0$, we already
...
is our IEEE-754 floating point result.  However, note that the code computes the exponent as:

{{< katex display >}}
\floor{-\dfrac{\fexp + 1}{2}} = -\dfrac{\fexp + 2}{2} = -\dfrac{\fexp}{2} - 1
{{< /katex >}}

This means the computed exponent will be one lower than it should be.  Therefore, if we let $m = 0.999999\ldots$ then:

{{< katex display >}}
2^\floor{-(\fexp + 1)/2}(1 + m) = 2^{-(\fexp + 2)/2}(1 + m) = 2^{-\fexp/2 - 1}(1 + 0.9999\ldots) \approx  2^{-\fexp/2} 
{{< /katex >}}

Which gets us closer to where we want to be.  Fortunately, as we'll see later, this is possible to do directly within the lookup table, no branching required.
...
Let's recap.  Let $r_\ffrac$ denote the fractional part of the mantissa for the initial guess.  Given:

{{< katex display >}}
 r_\fexp = \floor{-\dfrac{\fexp + 1}{2}}
{{< /katex >}}
...
r_\ffrac = \begin{cases}
   ...
   0.9999\ldots &\text{if } \fexp \text{ is even and } \ffrac = 0 \\
...
Because this is just supposed to be an initial guess, it's perfectly fine to not have all of the mantissa bits be correct.
We'd ideally like the most significant ones to be, though.

Notice that $r_\ffrac$ only depends on $\fexp$ for selecting the right case.  Otherwise, it is characterized 
entirely in terms of $\ffrac$.  In fact, we only need $r_\ffrac$ the least significant bit of $\fexp$ to select the case.
Also notice that if $\fexp$ is even then $\biased{\fexp}$ is odd, and vice versa.

Now, let's define two functions:

{{< katex display >}}
M_{\mathrm{odd}}(2^\fexp(1 + \ffrac)) = \frac{\sqrt{2}}{\sqrt{\frac{1 + \ffrac}}
{{< /katex >}}

{{< katex display >}}
M_{\mathrm{even}}(2^\fexp(1 + \ffrac)) = \frac{\sqrt{2}}{\sqrt{\frac{1 + \ffrac}}
{{< /katex >}}

{{< katex display >}}
M(2^\fexp(1 + \ffrac)) = ... odd or (even and not frac 0) ...
                         ... even (frac 0)
{{< /katex >}}

Then we can characterize the reciprocal square root of our input $v$ as:

{{< katex display >}}
\frac{1}{\sqrt{v}} \approx 2^{\floor{-(\fexp + 1)/2}}M(2^\fexp(1 + \ffrac))
{{< /katex >}}

Suppose we had the following lookup tables:

{{< katex display >}}
\mathrm{LUT}_{\mathrm{odd}}[i] = \frac{\sqrt{2}}{\sqrt{1 + \frac{i}{128}}}
{{< /katex >}}

{{< katex display >}}
\mathrm{LUT}_{\mathrm{even}}[i] = \frac{2}{\sqrt{1 + \frac{i}{128}}}
{{< /katex >}}
...
{{< katex display >}}
M_{\mathrm{odd}}(2^\fexp(1 + \ffrac)) \approx LUT_{\mathrm{odd}}[\floor{\ffrac2^{7}}]
{{< /katex >}}

{{< katex display >}}
M_{\mathrm{even}}(2^\fexp(1 + \ffrac)) \approx \mathrm{LUT}_{\mathrm{even}}[\floor{\ffrac2^{7}}]
{{< /katex >}}

OK, not bad -- we have two lookup tables. Recall that we can pick which LUT to use based on the least significant bit of the exponent.
Then let's put these back to back in memory:
...
So, if we were able to, we could say
{{< katex display >}}
M(v) \approx LUT[2^8\mathrm{lbit}(\fexp) + \floor{\ffrac2^{7}}]
{{< /katex >}}

When $\fexp$ is odd, $\biased{\fexp}$ is even and the lower 128 entries are used, or $\mathrm{LUT}_{\mathrm{odd}}$.
When $\fexp$ is even, $\biased{\fexp}$ is even and the upper 128 entries are used, or $\mathrm{LUT}_{\mathrm{even}}$.

Therefore, we can construct a lookup table to select which mantissa bits to obtain based on the least significant bit
of the biased exponent and the 7 most significant bits of the mantissa field.

#### LUT Generation analysis


TODO: REMEMBER THE MANTISSA SHIFT
...
First, the loop: the range $[0.5, 2) is iterated 256 times using this formula:
...
We can see the fixup we have discussed previously marked above.  Since the LUT entry is 8 bits wide, the best that can be done is to set it to `0xFF`.
...
Also note that TODO this means the initial guess will be at most a factor of $\sqrt{2}$ off (from exponent)
...
I've put the code on [Compiler Explorer](TODO) (TODO ALSO TL;DR) in case anyone wants to play around with it.
...
## Converting a non-zero real number to a `float64`{#ieee754-real}
...
However, to go the *other* way, that is, to take a non-zero real of the form $\pm2^\fexp(1 + \ffrac)$ and represent it as a `float64`, 
