...
obtain mantissa bits.  It is best illustrated with the following code:
...
There are just two things to point out before we proceed:

* The 11-bit exponent field is stored *biased* by the value $1023$.  This means to get the true exponent, you need to subtract it by $1023$.
* The mantissa actually represents a 53-bit value.  The lower 52-bits are as they appear in the mantissa field, and the most significant bit is implicitly $1$ (for our purposes)
...
Note that x86 is little-endian, meaning the mantissa bits come first in memory.
...
For budding developers out there, before you get obsessed with notions of efficiency or the like,
remember this:
...
Write your code with clarity first, and let the optimizer handle the rest until it just isn't cutting it anymore.
...
and it's *way* easier to read.  Remember if a human can understand your code, you bet the compiler can too.
...
The code is available on [GitHub] as well as a CMake project.
