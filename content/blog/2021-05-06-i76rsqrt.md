...
void generateLUT()
{
    for (uint32_t i = 0; i < 256; ++i)
    {
        ...
        //this has the effect of correctly rounding the mantissa within 8 bits using the "round up" rule, which is exactly what is stored in the LUT.
        uint32_t const high32bits_rounded_up = high32bits + 0x400;

        uint8_t const mantissa_high8bits_only = (high32bits_rounded_up >> 0xc) & UINT32_C(0xFF); //original assembly used signed shift, but doesn't matter

        //store the 8 bits of mantissa remaining
        LUT[i] = mantissa_high8bits_only;

        //LUT[i] contains the 8 most significant bits of the mantissa field, correctly rounded up.
...
{{< highlight nasm >}}
; Original I'76                     |   ;MSVC 2019, x86-Release with /arch:IA32, /fp:fast, /Oy
...
This is acceptable. The only real difference is the constant `1.0` is obtained via `fld1` rather than a constant in memory, and the function cleans it off the stack before returning.
 Comparing the LUT generated here with the one visible in x64dbg yielded identical results,
...
~~~
6a 68 67 66 64 63 62 60 5f 5e 5c 5b 5a 59 57 56 55 54 53 52 50 4f 4e 4d 4c 4b 4a 49 48 47 46 45  
44 43 42 41 40 3f 3e 3d 3c 3b 3a 39 38 37 36 35 34 34 33 32 31 30 2f 2f 2e 2d 2c 2b 2a 2a 29 28  
27 27 26 25 24 24 23 22 21 21 20 1f 1f 1e 1d 1c 1c 1b 1a 1a 19 18 18 17 16 16 15 15 14 13 13 12  
11 11 10 10 0f 0e 0e 0d 0d 0c 0c 0b 0a 0a 09 09 08 08 07 07 06 05 05 04 04 03 03 02 02 01 01 00  
ff fe fc fa f8 f6 f4 f2 f0 ef ed eb e9 e8 e6 e4 e2 e1 df de dc da d9 d7 d6 d4 d3 d1 d0 ce cd cb  
ca c8 c7 c5 c4 c3 c1 c0 bf bd bc bb b9 b8 b7 b6 b4 b3 b2 b1 b0 ae ad ac ab aa a8 a7 a6 a5 a4 a3  
a2 a1 a0 9f 9e 9c 9b 9a 99 98 97 96 95 94 93 92 91 90 8f 8f 8e 8d 8c 8b 8a 89 88 87 86 85 85 84  
83 82 81 80 7f 7f 7e 7d 7c 7b 7a 7a 79 78 77 76 76 75 74 73 73 72 71 70 70 6f 6e 6d 6d 6c 6b 6a
~~~

As you can see, there is a discontinuity at `0x80`, where it jumps from `00` to `ff`.
...
For our purposes, the 11-bit exponent field contains an integer value between $1$ and $2046$ inclusive.
...
stored in the exponent field.  For example, the exponent $0$ is represented with the biased exponent $1023$, and
the exponent $-1$ is represented using $1022$.

Another interesting aspect of this format is that the mantissa actually has 53 bits.  Only 52 are explicitly stored in the `float64` -- the most
significant bit is implicitly 1 (for our purposes).  That is, if we treat the above mantissa field as an integer $z$, then the actual value of the mantissa is:

{{< katex display >}}
1 + \frac{z}{2^{52}}
{{< /katex >}}

The mantissa then has a range of:

{{< katex display >}}
1 \leq 1 + \frac{z}{2^{52}} < 2
{{< /katex >}}

Putting this together, a normalized `float64` with biased exponent $\mathrm{bexp}$ and mantissa field $z$ represents the value:

{{< katex display >}}
2^{\mathrm{bexp} - 1023}\left( 1 + \frac{z}{2^{52}} \right)
{{< /katex >}}

For example, the value $1$ is represented as follows.  First, $1 = 2^0(1 + 0)$.  Therefore the unbiased exponent is $0$, and
the biased exponent is $0 + 1023 = 1023$.  The fractional part of the mantissa $z$ is $0$, as we exclude the implicit leading $1$.
Finally, the sign bit is $0$ because the quantity is positive:
...
Or, in hexadecimal notation, this is written as `0x3FF0000000000000`. `float64` is kind of nice because the sign
...
The next thing the code does is perform one iteration of Newton-Raphson approximation.
The Quake 3 `Q_rsqrt` function is commonly characterized as being one iteration of Newton-Raphson with a good
first guess.
...
This looks really close!  But what's the deal with this `1.00001` term in the code?  It appears it is just a slight manual adjustment
...
TODO NOMENCLATURE: adjust representation from (1 + m) to n to on paper?
...
This is represented with lines ... of the pseudocode...
...
well known outside of a few circles.  If I'm wrong about this please let me know. 

I'd like to conclude by saying that virtually no new code should ever use anything like this.
...
things working at an acceptable speed.  Bit-twiddling was the norm to get things done.  In the embedded space, this is still
probably the case -- but for budding developers out there, before you get obsessed with notions of efficiency or the like,
remember this:

**Always profile your code before attempting to optimize anything!**

Do measurements, do test runs, etc.  But don't go flying blind, because chances are you'll actually end up
hurting your own performance.  I can't count how many times I've seen people write code in a weird way because
they have some kind of misguided notion that it will get more performance, and then I found out that they never
actually bothered to measure it.

Use a good profiler like [vTune](TODO) or TODO to help you understand what parts
of your code are slow, and most importantly, *why*.  There are so, so many things it could be, and chances
are you won't be able to figure it out by just looking at the code itself.  Save yourself the trouble.
Write your code with clarity, and let the optimizer do the rest.

The code demonstrated in this article was designed to be clear from the get-go, and look at how well it optimized.
Even in 32-bit mode, MSVC optimizes the 64-bit values right out and basically transformed it into the original code,
and it's *way* easier to read.  I encourage you write with clarity in mind first -- because if a human can understand it,
you bet the compiler can too.
...
TODO: I've put the code on [Compiler Explorer](TODO) (ALSO TL;DR)

I've put the code on [GitHub](TODO) in case anyone wants to play around with it.  Do note that building on different compilers
could give [slightly different results](#fp-assoc) if fast math is enabled.  It's a CMake project and should work on all major platforms.
Presets are included for Clang and MSVC 2019.
